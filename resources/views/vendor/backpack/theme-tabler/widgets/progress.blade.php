...
<div class="{{ $widget['class'] ?? 'card' }} @if($widget['ribbon'] ?? false) border-start-0 @endif">
    ...
    @if (isset($widget['footer_link']))
        <div class="card-footer px-3 py-2">
            <a class="btn-block card-text d-flex justify-content-between align-items-center" href="{{ $widget['footer_link'] ?? '#' }}"><span class="small font-weight-bold">{{ $widget['footer_text'] ?? 'View more' }}</span><i class="ti ti-angle-right"></i></a>
        </div>
    @endif
  </div>
